...
abstract class WPCOM_JSON_API_Render_Endpoint extends WPCOM_JSON_API_Endpoint {
	...
	/*
	 * Figure out what scripts and css to load.
	...
	function process_render( $callback, $callback_arg ) {
		...
		// initial scripts & css (to subtract)
		ob_start();
		wp_head();
		wp_footer();
		ob_end_clean();
	...
	/**
	 * Takes the list of css and scripts and adds them to the JSON response
	 */
	...
	/**
	 * Returns the 'version' string set by the shortcode so different versions of scripts/css can be loaded
	 */
