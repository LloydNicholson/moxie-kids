...
class A8C_WPCOM_Masterbar {
	/**
	 * Use for testing changes made to remotely enqueued scripts and css on your sandbox.
	...
	public function init() {
		...
		if ( $this->is_rtl() ) {
			// Extend core WP_Admin_Bar class in order to add rtl css.
		...
		if ( Jetpack::is_module_active( 'notes' ) && $this->is_rtl() ) {
			// Override Notification module to include RTL css.
	...
	public function remove_core_styles() {
		/*
		 * Notifications need the admin bar css,
	...
	private function wpcom_static_url( $file ) {
		if ( ! empty( $this->sandbox_url ) ) {
			// For testing undeployed changes to remotely enqueued scripts and css.
