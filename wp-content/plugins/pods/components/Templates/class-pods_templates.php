...
class Pods_Templates_Frontier {
		...
		if ( in_array( $screen->id, $this->plugin_screen_hook_suffix, true ) ) {
			...
			if ( ! empty( $configfiles ) ) {
				...
				foreach ( $configfiles as $key => $fieldfile ) {
					...
					if ( ! empty( $group['css'] ) ) {
						foreach ( $group['css'] as $style ) {
							wp_enqueue_style( $this->plugin_slug . '-' . strtok( $style, '.' ), $this->get_url( 'assets/css/' . $style, __FILE__ ) );
						}
					}
				}
			}
			wp_enqueue_style( $this->plugin_slug . '-admin-css', $this->get_url( 'assets/css/panel.css', __FILE__ ), array(), self::VERSION );
	...
	public function add_metaboxes( $slug, $post = false ) {
		...
		wp_enqueue_style( $this->plugin_slug . '-view_template-css', $this->get_url( 'assets/css/css-view_template.css', __FILE__ ), array(), self::VERSION );
		...
		wp_enqueue_style( $this->plugin_slug . '-pod_reference-css', $this->get_url( 'assets/css/css-pod_reference.css', __FILE__ ), array(), self::VERSION );
	...
	public function render_element( $atts, $content, $slug, $head = false ) {
		...
		if ( ! empty( $head ) ) {
			...
			// process headers - CSS
			if ( file_exists( $this->get_path( __FILE__ ) . 'assets/css/css-' . $slug . '.php' ) ) {
				ob_start();
				include $this->get_path( __FILE__ ) . 'assets/css/css-' . $slug . '.php';
				$this->element_header_styles[] = ob_get_clean();
				add_action( 'wp_head', array( $this, 'header_styles' ) );
			} else {
				if ( file_exists( $this->get_path( __FILE__ ) . 'assets/css/css-' . $slug . '.css' ) ) {
					wp_enqueue_style( $this->plugin_slug . '-' . $slug . '-css', $this->get_url( 'assets/css/css-' . $slug . '.css', __FILE__ ), array(), self::VERSION );
				}
			}
			// process headers - JS
