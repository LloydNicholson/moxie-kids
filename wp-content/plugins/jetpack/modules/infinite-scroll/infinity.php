...
class The_Neverending_Home_Page {
	...
	function action_wp_footer() {
		...
		/**
		 * Filter the list of css already present on the page.
		 *
		 * @module infinite-scroll
		 *
		 * @since 2.1.2
		 *
		 * @param array $styles Array of css present on the page.
		 */
	...
	function filter_infinite_scroll_results( $results, $query_args, $wp_query ) {
		...
		$initial_styles = isset( $_REQUEST['css'] ) && is_array( $_REQUEST['css'] ) ? array_map( 'sanitize_text_field', $_REQUEST['css'] ) : false;

		if ( is_array( $initial_styles ) ) {
			global $wp_styles;

			// Identify new css needed by the latest set of IS posts
			$new_styles = array_diff( $wp_styles->done, $initial_styles );

			// If new css are needed, extract relevant data from $wp_styles
			if ( ! empty( $new_styles ) ) {
				$results['css'] = array();
				...
				foreach ( $new_styles as $handle ) {
					...
					array_push( $results['css'], $style_data );
				}
			}
		}
		...
		/**
		 * Filter the additional css required by the latest set of IS posts.
		 *
		 * @module infinite-scroll
		 *
		 * @since 2.1.2
		 *
		 * @param array $results['css'] Additional css required by the latest set of IS posts.
		 * @param array|bool $initial_styles Set of css loaded on each page.
		 * @param array $results Array of Infinite Scroll results.
		 * @param array $query_args Array of Query arguments.
		 * @param WP_Query $wp_query WP Query.
		 */
		$results['css'] = apply_filters(
