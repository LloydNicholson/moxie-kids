 ...
 * the options and outputting any appropriate css
 * for the theme.
 *
...
if ( ! class_exists( 'EGF_Frontend' ) ) :
	class EGF_Frontend {
		 ...
		 * that our css are outputted as late as
		 * possible.
		 * 
		 * @since 1.2
		 * @version 1.4.4
		 ...
		 * inline css relevant to each font option.
		 *
		...
		public function output_styles() {
			...
			if ( ! isset( $wp_customize ) ) {
				echo '<style id="tt-easy-google-font-css" type="text/css">';
			}
			...
			foreach ( $options as $key => $value ) {
				...
				// Check if css css should be forced.
				...
				if ( isset( $wp_customize ) && ! empty( $options[ $key ] ) ) {					
					...
					// Output media query, selector and css
					// for this font control.
					echo $this->get_opening_media_query( $key );
					
					// Only output css if a selector exists.
		 ...
		 * css in their own <style> tag we are able to use the
		 ...
		 * Used by this class to output css in the
		 * frontend.
		 * 
