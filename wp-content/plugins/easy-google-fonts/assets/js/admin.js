...
;( function($, window, document, undefined) {
	$.fn.themeFontManager = function() {
		 ...
		 *
		 * @description - Constructs an AJAX request to 
		 *     turn force css on/off for a control. 
		...
		api.registerEditEvents = function() {
			...
			$( '#custom-control-name, #tt-font-tags, #control-force-css' ).on( 'change', function(){
				api.registerChange();
			});
			 ...
			 */
			 $( '#save_control_header, #save_control_footer' ).on( 'click', function() {
				...
				var forceStyles      = $( '#control-force-css' ).prop('checked');
		...
		api.registerManagementEvents = function() {
			 ...
			 */
			$( '#font-controls-table .tt-force-css' ).on( 'change', function() {
